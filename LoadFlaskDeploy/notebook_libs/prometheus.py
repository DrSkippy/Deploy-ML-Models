...
query_tags = '{image="localhost:32000/load-model-service:latest", pod=~"load-model-service-.*"}'
query_duration = '[{time_span}m:1m]'
...
def experiment_pod_data_query(time_span=200):
    q_dur = query_duration.format(time_span=time_span)
    q_dict = {
        "cpu": f'rate(container_cpu_usage_seconds_total{query_tags}[1m]){q_dur}',
        "memory": f'rate(container_memory_usage_bytes{query_tags}[1m]){q_dur}'
    }
